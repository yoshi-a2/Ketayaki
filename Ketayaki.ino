...
void funcShiftReg(char x){
  ...
  shiftOut(dsPin, srclkPin, LSBFIRST, seven_leds[y]); //シフト演算を使って点灯するLEDを選択
  digitalWrite(rclkPin, HIGH);                        //送信終了後RCLKをHighにする
}


void funcShiftReg_dig3(char x){
  if(x == '0'){
    y = 0;
  }
  if(x == '1'){
  ...
  digitalWrite(rclkPin, LOW);                         //送信中のRCLKをLowにする
  shiftOut(dsPin, srclkPin, LSBFIRST, seven_leds_dig3[y]); //シフト演算を使って点灯するLEDを選択
  digitalWrite(rclkPin, HIGH);                        //送信終了後RCLKをHighにする
}
